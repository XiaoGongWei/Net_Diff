      subroutine gpt2_1w (dmjd,dlat,dlon,hell,nstat,it,p,T,dT,Tm,e,ah,
     .       aw,dla,undu)
...
c% It is based on a 1 x 1 degree external grid file ('gpt2_1w.grd') with mean
...
c%     .          ah(64800),aw(64800),undu(64800)
     ...
     .         awgrid(64800,5) ,dlagrid(64800,5),Tmgrid(64800,5)
...
c% read gridfile
      inquire(file=trim(GPT2GridDir)//'gpt2_1wA.grd',exist=alive) !Added by Yize Zhang
      if (.not. alive) then
          write(*,*) "GPT2 grid file: gpt2_1wA.grd doesn't exist!"
          pause
          stop
      end if
c%      open(11,file='gpt2_1wA.grd')  ! Changed  by Yize Zhang, see below
c%   % use the 1 degree grid (GP)
      GPTID=FileID_Mark
      FileID_Mark=FileID_Mark+1
      open(unit=GPTID, file=trim(GPT2GridDir)//'gpt2_1wA.grd')

c% read first comment line
      read (GPTID,'(a80)') line

c% loop over grid points
c% 64800 for the 1 degree grid (GP)
      do n = 1,64800
...
c% loop over stations
     ...
     .           Tmgrid(ix,4)*coshy + Tmgrid(ix,5)*sinhy
		 ...
		 !% water vapor pressure in hPa - changed by GP
		  ...
		  e(k) = e0*(100.d0*p(k)/p0)**(dla(k)+1) !% on the station height - (14) 
