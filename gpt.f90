! -------------------------------------------------------------------------
! Purpose  : This Function is to read a priori station coordinates/velocities
!            
!            
! 
...
!      In fact, this is refer from gpt  http://ggosatm.hg.tuwien.ac.at/DELAY/SOURCE/gpt.f
!############################ end_of_header ##############################################

  subroutine gpt (dmjd,dlat,dlon,dhgt,pres,temp,rhumity,undu)
...
!  rhumity = 0.60 added by G.Gendt
!  Johannes Boehm, 2006 June 12
!  rev 2006 June 16: geoid undulation is accounted for
!
!  Reference:
!  J. Boehm, and H. Schuh,
!  Global Pressure and Temperature (GPT): A spherical harmonic expansion 
...
! Parameters :
real (8)  ,intent(in)   :: dmjd          ! modified julian date
real (8)  ,intent(in)   :: dlat,dlon     ! latitude,longitude in radians
real (8)  ,intent(in)   :: dhgt          ! ellispoidal height in m
real (8)  ,intent(out)  :: pres          ! pressure in hPa
real (8)  ,intent(out)  :: temp          ! temperature in Celsius
real (8)  ,intent(out)  :: rhumity       ! relative humity 
real (8)  ,intent(out)  :: undu          ! Geoid undulation in m (from a 9x9 EGM based model)
...
!! local variables
 ...
 real (8)    :: doy,t,sum,hort
 real (8)    :: apm,apa,pres0
 real (8)    :: atm,ata,temp0
 integer (4) :: n,m,i,j,k,ir
