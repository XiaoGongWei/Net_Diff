...
!            if ( any(STA%STA(k)%TrueCoor==0.d0) .or. ((Pos_State=='K') .and. .not.(STA%STA(k)%flag_InitialCoor))  ) then
            if ( any(STA%STA(k)%TrueCoor==0.d0) .or. (Pos_State=='K' .and. any(dabs(STA%STA(k)%TrueCoor-STA%STA(k)%XYZ)>20.d0))  ) then
...
!                    end if
                ...
                if ( (index(ObsCombine,"P1")/=0) .or. (index(ObsCombine,"G1")/=0) )then
                    ...
                    if (P1 /=0.0d0) Range=P1-Ion1  ! use P1 rather than %P1(i), because P1 have corrected by DCB
                    if (L1 /=0.0d0) then
                ...
                elseif ( (index(ObsCombine,"P2")/=0) .or. (index(ObsCombine,"G2")/=0) )then
                    ...
                    if (ObsData%P2(i) /=0.0d0) Range=ObsData%P2(i)-Ion1  ! If L2L3 combination, frequency is wrong
            ...
            if ((STA%STA(k)%SKD=="F") .or. (STA%STA(k)%SKD=="f")) then
                ...
                if (If_Est_Iono .and. (index(ObsCombine,"P1") /=0 .or. index(ObsCombine,"P2") /=0 .or. index(ObsCombine,"P3") /=0)) then
                    do i=1, SatNum
                        ...
                        elseif (InvN(ParaNum-SatNum+i,ParaNum-SatNum+i)>0.d0 .and. any(A(1:N, ParaNum-SatNum+i)==0.d0) ) then
                            ! ********* Eliminate the ionosphere and ambiguity parameter of unobserved satellite *********
                            ! ! If not observed more than 10 minutes
                            if ((CycleSlip(k)%CS(i)%LastWeek-ObsWeek)*604800.0d0+CycleSlip(k)%CS(i)%LastSow-ObsSec<-600.d0) then
                                if (ADmethod=='LS') then
                                    call Elimi_Para(Nbb,U,SatNum+ParaNum,ParaNum-SatNum+i)
                                    call Elimi_Para(Nbb,U,SatNum+ParaNum,ParaNum+i)
                                elseif (ADmethod=='KF') then
                                    call KF_Change(InvN, dx,SatNum+ParaNum, ParaNum-SatNum+i, 'dds')
                                    call KF_Change(InvN, dx,SatNum+ParaNum, ParaNum+i, 'dds')
                                end if
                            end if
...
!                    if (outlier<0.10d0) outlier=0.10d0
            ...
            if (If_Est_Iono .and. (index(ObsCombine,"P1") /=0 .or. index(ObsCombine,"P2") /=0 .or. index(ObsCombine,"P3") /=0)) then
                ...
                do i=ParaNum-SatNum+1, ParaNum
                    if (dx(i)/=0.d0) then
                        write(LogID,'(I6,F8.2)',advance='no') i-ParaNum+SatNum, dx(i)
                    end if
                end do
                write(LogID,'(A)')
            end if
