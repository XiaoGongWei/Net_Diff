...
!
! PURPOSE:
!    Read the Header of BEIDOU navigation ephemeris file
!
! INPUT:
...
subroutine ReadNavHead_C(NavFile)
use MOD_FileID
use MOD_NavHead
implicit none
    ...
    if (.not. alive) then
        write(*,*) "COMPASS Navigation flie: """//trim(NavFile)//""" doesn't exist!"
        pause
        stop
    end if
    ...
    do while(.true.)
        read(NavID_C,fmt="(A)",iostat=error) line
        if (error /=0) exit
        keyword=line(61:80)
        if (index(keyword,"RINEX VERSION / TYPE") /= 0) then
            read(line,fmt="(5X,I1)") NavHead_C%Version
