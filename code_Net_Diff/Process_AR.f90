! ===========================================
!  This is the main processing  part for PPP_AR
!
!
! WRITTEN BY: Yize Zhang, zhyize@163.com, TUMSAT & SHAO
! ===============  End of Header  ==============

subroutine Process_AR
use MOD_constant
use MOD_FileID
use MOD_FileDir
use MOD_NavHead
...
!    DCBFile='E:\gbm\cnt20043.bia'
    ...
    do while(flag)
        ...
        ! ************** Get CNES Bias value of current epoch *************
        if ( (Obsweek-Bias%week)*604800.0d0+Obssec-Bias%sow > 0.0d0) then
            call Get_CNESBias(Obsweek,Obssec)
        end if
...
!             Rec_Clk=0.d0
            ...
            do i=1,ObsData%PRNS
                ...
                elseif (freq_comb=='L2L3') then
                    ...
                    P1=P1+Bias%P2(PRN)*c  ! Add code and phase bias for PPP-AR
                    P2=P2+Bias%P3(PRN)*c
                    L1=L1+Bias%L2(PRN)*f1
                    L2=L2+Bias%L3(PRN)*f2
                end if
...
!                    end if
                ...
                if (CycleSlip(k)%Slip(PRN)==1) then
                    ...
                    elseif (ADmethod=='KF') then
                        call KF_Change(InvN,dx,SatNum+ParaNum,PRN+ParaNum,'clk')
                    end if
                    Amb(PRN,k)=0.d0
...
!                        InvN(PRN+ParaNum, PRN+ParaNum)=InvN(PRN+ParaNum, PRN+ParaNum)+1.d-8*Interval
                ...
                if (Amb(PRN,k)==0.d0) then
                    Amb(PRN,k)=0.d0 ! Phase-s-STD+Sat_Clk*c-c*Rec_Clk+Rela -dx_windup*c/(f1+f2) ! (Phase-dx_windup*lambda-Range)   !  In meter    ! randa=c/(f1+f2)
                end if
...
!                    A(N, PRN+ParaNum)=PP(N)*c/(f1+f2)   ! IF ambiguity in distance: (c*f1*N1-c*f2*N2)/(f1^2-f2^2)=c/(f1+f2)*N1+c*f2/(f1^2-f2^2)*(N1-N2)
                    ...
                    L(N)=Phase-s-STD+Sat_Clk*c-c*Rec_Clk+Rela-Bias%WL_amb(PRN)*c*f2/(f1**2-f2**2) -dx_windup*lambda
