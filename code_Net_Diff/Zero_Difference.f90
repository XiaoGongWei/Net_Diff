...
implicit none
    ...
    dist=dsqrt(DOT_PRODUCT((AppCoor-STA%STA(STA%Num)%TrueCoor),(AppCoor-STA%STA(STA%Num)%TrueCoor)))
...
!        if ( (L1/=0.d0) .and. (L3/=0.d0) .and. (a1*f1+a2*f2/=0.d0)  ) ZD%W4(N)=(a1*f1*ZD%L1(N)+a2*f2*LC)/(a1*f1+a2*f2)
        write(unit=LogID,fmt='(A6,1X,A1,I2,2F8.2,2F13.2,E15.7,3F7.2, 2X, 2F7.2,4F13.2)') 'PRN',System,PRN_S,Ele, Azi,P1,  &
