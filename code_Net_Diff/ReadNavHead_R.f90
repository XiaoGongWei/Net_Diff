...
subroutine ReadNavHead_R(NavFile)
use MOD_FileID
use MOD_NavHead
implicit none
    ...
    if (.not. alive) then
        write(*,*) "GLONASS Navigation flie: """//trim(NavFile)//""" doesn't exist!"
        pause
        stop
    end if
    ...
    open(unit=NavID_R, file=NavFile,action="read")
    
    do while(.true.)
        read(NavID_R,fmt="(A)",iostat=error) line
        if (error /=0) exit
        keyword=line(61:80)
        if (index(keyword,"RINEX VERSION / TYPE") /= 0) then
            read(line,fmt="(5X,I1)") NavHead_R%Version
