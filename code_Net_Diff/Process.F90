...
!                        InvN(PRN+ParaNum, PRN+ParaNum)=InvN(PRN+ParaNum, PRN+ParaNum)+1.d-8*Interval
                ...
                if (PDOP>MaxPDOP) then
                    write(unit=LogID,fmt='(5X,A30,F5.1)') 'PDOP超限，不计算。PDOP=', PDOP
                    cycle
                end if
...
!                    if (outlier<0.15d0) outlier=0.15d0
                    ...
                    elseif ( (dabs(maxV)>outlier)  ) then ! 适用于相位
                        ...
                        Ad_Flag=.true.
                        write(unit=LogID,fmt='(5X,A5,3F15.3,I3,F10.3,A8)') '!!!dx', dx(1:3),PRNPRN(maxL), maxV,'outlier'
                    end if
                    write(unit=LogID,fmt='(5X,A5,3F15.3,I3,F10.3)') '!!dx',dx(1:3),PRNPRN(maxL), maxV
