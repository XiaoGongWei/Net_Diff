...
C
      ...
      real*8  ETa,ETb
      DIMENSION SS(3),CVAL(400),PVSUN(6),zips(2),pnut(4)
      data zips/2*0.d0/
...
C          (SEE THE DISCUSSION IN THE SUBROUTINE STATE)
      ...
      ET2(2)=ET2Z(2)
c
      ENTRY DDPLEPH(ETa,ETb,NTARG,NCENT,RRD)
      ET2(1)=ETa     
      ET2(2)=ETb    
cc end of new entry
c
...
! J.Chen  DE405  NRECL adapted from 4 to 1
!      NRECL=1
!      KSIZE=2036
!! J.Chen de200
      NRECL=1
...
C ************************************************************************
      ...
      NAMFIL= 'Planet.bin'  ! 'jpl_eph' !
      NAMFIL=PlanetFile
C      lreturn=l_openfile(NAMFIL,NRFILE,cstatus='o',cformat='u' 
C     *           ,caccess='d',irecl=IRECSZ,lstop=.true.)
...
!      write(*,*) "time NR.NE.NRL NCOEFFS ", T(1),NR,NRL,NCOEFFS
      IF(NR.NE.NRL) THEN
        NRL=NR
        READ(NRFILE,REC=NR,ERR=99)(BUF(K),K=1,NCOEFFS)
      ENDIF
!      print*,"buf ",(BUF(K),K=1,NCOEFFS)
      IF(KM) THEN
      T(2)=SS(3)*86400.D0
      AUFAC=1.D0
      ELSE
...
C
cc
cc Ge Maorong, changed for getting special constants
c
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
!
!! get values of specified constants
        do i=1,n
          val(i)=0.d0
          do j=1,ncon
            if(trim(nam(i)).eq.trim(cnam(j))) then
              val(i)=cval(j)
              exit
            endif 
          enddo
